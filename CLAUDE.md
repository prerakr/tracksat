...
- **No HTML dom-overlay**: Quest's browser doesn't reliably composite one, and when present it forwards controller `select` as DOM pointer events that fight in-scene raycasting. All XR UI (category/zone toggles, passthrough toggle, locate, exit) lives in **`lib/xrMenu.ts`**'s `XRMenu` — a single canvas-textured plane hit-tested via the controller ray, driven by the same `activeCategories`/`visibleZones`/`onToggleCategory`/etc. props `App.tsx` already threads into the non-XR UI. `XRMenu.updateHoverFromRay` runs every frame per controller to highlight whichever button the ray is over (tracked by a structural key, not the `Row` object, since `redraw()` rebuilds `rows` from scratch) — it only actually repaints the canvas/re-uploads the texture when the hovered button changes, so continuous per-frame calls stay cheap. Firing still happens on `selectstart` via `hitFromRay`.
...
- **Ray termination**: each controller's ray is rescaled every frame (`ray.scale.z`) to stop exactly at the nearest of the menu, the move handle, or the globe surface (analytic ray/sphere test, `raySphereDistance`) instead of always drawing a fixed `RAY_LEN`-metre line through everything; a small marker mesh (a child of the controller, so its position is just local `(0, 0, -hitDistance)`) sits at that point as a visible cursor. This is also what drives the menu hover highlight above — without it there was no way to tell where the ray was landing well enough to aim a pinch at a button from a distance.
- **Perf on standalone headsets**: `renderer.xr.setFoveation`/`setFramebufferScaleFactor` (`XR_FOVEATION`/`XR_FRAMEBUFFER_SCALE`) are set before `setSession` to trade a bit of peripheral sharpness for frame time on Quest's fill-rate-limited GPU. `ctrlWorldPos` takes an output `Vector3` (module-level scratch vectors `_cp`/`_p0`/`_p1`, reused since the render loop and `onSelectStart` never hold a reference past their own synchronous scope) instead of allocating a new one — grabs were doing up to 4 `new THREE.Vector3()`s per frame, which is enough GC churn to read as stutter on Quest's shared GPU/CPU memory.
